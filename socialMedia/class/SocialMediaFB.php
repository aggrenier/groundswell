 ...
 */
class SocialMediaFB extends Facebook {
  /**
  ...
  function publish($publication, string $page_id = NULL) {
    ...
    if (empty($this->type)) {
      throw new ErrorException(t('The publication array must contain publication type.'), SocialMediaFB::missing_param, WATCHDOG_ERROR);;
    }
    // Error generation
    if (!isset($publication['params'])) {
      throw new ErrorException(t('The publication array must contain publication parameters.'), SocialMediaFB::missing_param, WATCHDOG_ERROR);
    }
    ...
    try {
      ...
      // Throw an exception
      throw new ErrorException(t('Facebook SDK threw an error: %error', array('%error' => $e)), SocialMediaFB::sdk_error, WATCHDOG_ERROR);
    }
  }
  ...
  public function getPagesData($account_id, $account_access_token) {
    ...
    if (empty($account_access_token)) {
      throw new ErrorException(t('Cannot ask for user accounts without an access token.'), SocialMediaFB::missing_param, WATCHDOG_ERROR);
    }
    try {
  ...
  public function getSession() {
    return new SocialMediaFBSession();
  }
  ...
  public function setPrivacy($privacy_code) {
    if (!in_array($privacy_code, array('ALL_FRIENDS', 'EVERYONE', 'SELF', 'FRIENDS_OF_FRIENDS'))) {
      throw new ErrorException(t('The privacy code you specified is invalid.'), SocialMediaFB::incorrect_param, WATCHDOG_ERROR);
    }
  ...
  public function setType($type) {
    if (!in_array($type, array_keys($this->publication_types))) {
      throw new ErrorException(t('The publication type you specified is invalid.'), SocialMediaFB::incorrect_param, WATCHDOG_ERROR);
    }
    $this->type = $type;
    return $this;
  }
